...
impl Path {
    ...
    pub fn follow<'a>(&'a mut self, ast: &'a Ast) -> Option<&'a NodeInfo> {
        match self.dirs.pop_front() {
            None => info(ast),
            Some(PathDir::Child) => match child(ast) {
                Some(child) => self.follow(child),
                None => None,
            },
            Some(PathDir::Sibling) => match sibling(ast) {
...
pub fn info(ast: &Ast) -> Option<&NodeInfo> {
    match ast.as_ref() {
        Tree::Node(TreeNode { info, .. }) => Some(info),
        Tree::Nil => None,
    }
}
...
pub fn sibling(ast: &Ast) -> Option<&Ast> {
    match ast.as_ref() {
        Tree::Node(TreeNode { sibling, .. }) => Some(sibling),
        Tree::Nil => None,
    }
}
...
pub fn child(ast: &Ast) -> Option<&Ast> {
    match ast.as_ref() {
        Tree::Node(TreeNode { child, .. }) => Some(child),
        Tree::Nil => None,
    }
}
...
pub fn node(node_info: NodeInfo, child: Ast, sibling: Ast) -> Ast {
    Rc::new(Tree::Node(TreeNode { info: node_info, child, sibling }))
}
